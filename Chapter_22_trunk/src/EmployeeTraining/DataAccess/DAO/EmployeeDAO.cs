...
namespace DataAccess.DAO {
    public class EmployeeDAO : BaseDAO {
        ...
        private const String LOGIN_HASH = "@loginhash";
        private const String FK_USER_ROLE = "@fkuserrole";
        ...
        private const String INSERT_EMPLOYEE =
            "INSERT INTO tbl_Employee " +
                    "(FirstName, MiddleName, LastName, Birthday, Picture, HireDate, IsActive, Username, LoginHash, FK_UserRole) " +
            "VALUES (" + FIRST_NAME + ", " + MIDDLE_NAME + ", " + LAST_NAME + ", " + BIRTHDAY + "," +
                         PICTURE + ", " + HIRE_DATE + ", " + IS_ACTIVE +  ", " + USER_NAME + ", " + LOGIN_HASH + 
                         ", " + FK_USER_ROLE + ") " +
        ...
        private const String SELECT_EMPLOYEE_BY_USERNAME =
            SELECT_ALL_COLUMNS +
            "FROM tbl_Employee " +
        ...
        public List<EmployeeVO> SelectAllEmployees() {
            ...
            try {
                ...
                while (reader.Read()) {
                    list.Add(FillInEmployeeVO(reader, trainingDAO, addressDAO, emailAddressDAO, phoneNumberDAO, userRoleDAO));
                }
            }
            catch (Exception e) {
        ...
        public EmployeeVO SelectEmployee(int id) {
            ...
            try {
                ...
                reader = Database.ExecuteReader(command);
                if (reader.Read()) {
                    vo = FillInEmployeeVO(reader, trainingDAO, addressDAO, emailAddressDAO, phoneNumberDAO, userRoleDAO);
                }
                else {
        ...
        public EmployeeVO SelectEmployee(string username) {
            LogDebug("Entering SelectEmployee() method for username = " + username);
            EmployeeVO vo = null;
            IDataReader reader = null;
            TrainingDAO trainingDAO = new TrainingDAO();
            AddressDAO addressDAO = new AddressDAO();
            ...
            try {
                DbCommand command = Database.GetSqlStringCommand(SELECT_EMPLOYEE_BY_USERNAME);
                Database.AddInParameter(command, USER_NAME, DbType.String, username);
                reader = Database.ExecuteReader(command);
                if (reader.Read()) {
                    vo = FillInEmployeeVO(reader, trainingDAO, addressDAO, emailAddressDAO, phoneNumberDAO, userRoleDAO);
                }
                else {
                    throw new DBException("No employee found for username: " + username);
                }
            }
            catch (Exception e) {
                LogError("Error getting employee by username " + username, e);
                throw new DBException("Error getting employee by username " + username);
            }
            finally {
                base.CloseReader(reader);
            }
        ...
        public EmployeeVO InsertEmployee(EmployeeVO vo) {
            ...
            try {
                ...
                Database.AddInParameter(command, LOGIN_HASH, DbType.String, vo.LoginHash);
                Database.AddInParameter(command, FK_USER_ROLE, DbType.Int32, vo.UserRole.RoleID);
        ...
        private EmployeeVO FillInEmployeeVO(IDataReader reader, TrainingDAO trainingDAO,
                                            ...
                                            PhoneNumberDAO phoneNumberDAO, UserRoleDAO userRoleDAO) {
            ...
            vo.Username = reader.GetString(7);
            vo.LoginHash = reader.GetString(8);
            vo.UserRole = userRoleDAO.SelectUserRole(reader.GetInt32(9));

            if (!reader.IsDBNull(10)) {
                vo.Picture = (byte[])reader.GetValue(10);
            }
